...
function SWEP:SetIronsights(b,pl)

if CLIENT or (not pl) or pl:IsNPC() then return end
	...
	if self.UseScope then -- If we have a scope, use that instead of ironsights
		if b then
			...
			timer.Simple(IRONSIGHT_TIME, self.SetScope, self, true, pl)
		else
			self:SetScope(false, pl)
		end
	end

end

function SWEP:SetScope(b,pl)
	...
	if b then
		if IsValid(pl) then pl:DrawViewModel(false) end
		if PlaySound then
			self.Weapon:EmitSound(sndZoomIn)
		end
	else
		if IsValid(pl) then pl:DrawViewModel(true) end
		if PlaySound then
